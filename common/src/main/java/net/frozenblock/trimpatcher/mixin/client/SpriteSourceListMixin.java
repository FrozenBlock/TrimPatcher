...
import net.frozenblock.trimpatcher.TrimPatcher;
...
public class SpriteSourceListMixin {
	...
	private static SpriteSourceList trimPatcher$blendTrimSources(List<SpriteSource> sources, Operation<SpriteSourceList> original) {
		...
		for (SpriteSource spriteSource : sources) {
			if (spriteSource instanceof PalettedPermutations palettedPermutations && palettedPermutations.paletteKey().equals(TrimPatcher.TRIM_PALETTE_KEY)) {
		...
		for (PalettedPermutations palettedPermutations : trimSources) {
			...
			for (Map.Entry<String, Identifier> permutation : palettedPermutations.permutations().entrySet()) {
				...
				if (!permutations.containsKey(key)) {
					...
					TrimPatcher.addFoundOverlayMaterial(key);
				}
			}
		}

		final PalettedPermutations combinedTrims = new PalettedPermutations(textures, TrimPatcher.TRIM_PALETTE_KEY, permutations);
